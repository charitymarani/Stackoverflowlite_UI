...
from app.api.v1.question import views
...
def accept_answer(self):
    ...
    with self.client:
        ...
        self.assertTrue(response_data['message'] ==
                        'Answer posted successfully')
        self.assertEqual(resp.status_code, 201)
        accept_resp = self.client.patch('/api/v1/questions/1/answers/1/accept',
                                        headers=dict(
                                            Authorization='Bearer ' + a_token),
                                        data=json.dumps(dict(
                                            accepted=True)),
                                        content_type='application/json')
        respo_data = ast.literal_eval(accept_resp.data)
        print(respo_data)
    ...
    def tearDown(self):
        ...
        del self.all_questions[:]
        del self.all_answers[:]
